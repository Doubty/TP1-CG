...
void Curves::Update()
{
    ...
    {
        // nova quantidade de vértices = 2n-2
        uint quantidade_apos_itereacao = 2 * count - 2;
        float x, y;

        // rodada do algoritmo
        if (quantidade_apos_itereacao < MaxVertex)
        {

            uint controlador_indice = 0;
            for (uint cont = 0; cont < count - 1; cont++)
            {
                x = (3.0f / 4.0f) * vertices[cont].Pos.x + (1.0f / 4.0f) * vertices[cont + 1].Pos.x;
                y = (3.0f / 4.0f) * vertices[cont].Pos.y + (1.0f / 4.0f) * vertices[cont + 1].Pos.y;
                vetor_auxiliar[controlador_indice] = { XMFLOAT3(x, y, 0.0f), XMFLOAT4(Colors::Yellow) };
                controlador_indice++;

                x = (1.0f / 4.0f) * vertices[cont].Pos.x + (3.0f / 4.0f) * vertices[cont + 1].Pos.x;
                y = (1.0f / 4.0f) * vertices[cont].Pos.y + (3.0f / 4.0f) * vertices[cont + 1].Pos.y;
                vetor_auxiliar[controlador_indice] = { XMFLOAT3(x, y, 0.0f), XMFLOAT4(Colors::Yellow) };
                controlador_indice++;

            }
            count = quantidade_apos_itereacao;


            // colocando todos os valores do auxiliar no original
            if (count < MaxVertex)
            {
                memmove(vertices, vetor_auxiliar, count * sizeof(Vertex));
            }
        ...
        else
        {
            ...
            // colocando todos os valores do original na cor branca

            for (uint cont = 0; cont < count; cont++)
            {
                vertices[cont].Color = XMFLOAT4(Colors::White);
            }
            graphics->Copy(vertices, geometry->vertexBufferSize, geometry->vertexBufferCPU);
    ...
    if (input->KeyPress(VK_LBUTTON))
    {
        ...
        // colocando um vértice branco num ponto recem clicado 
        if (count < MaxVertex)
        {
            vertices[count] = { XMFLOAT3(x, y, 0.0f), XMFLOAT4(Colors::White) };
            count++;
        }
    ...
    if (input->KeyPress(VK_DELETE))
    {
        ...
        // marcando o indice do vetor original para zero
        count = 0;
        graphics->Copy(vertices, geometry->vertexBufferSize, geometry->vertexBufferCPU);
